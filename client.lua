			...
			if(args[1] == "pilot") then
				...
				if(IsWaypointActive()) then
					if(pilot) then
						...
						TaskVehicleDriveToCoord(GetPlayerPed(-1), GetVehiclePedIsIn(GetPlayerPed(-1), 0), waypoint["x"], waypoint["y"], waypoint["z"], 50.0, 10.0, GetHashKey(GetVehiclePedIsIn(GetPlayerPed(-1), 0)), 786603, 1.0, 1)
					...
					exports['okokNotify']:Alert("AUTOPILOT", "SET A WAYPOINT!", 2000, 'error')
				end
